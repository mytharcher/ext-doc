...
<xsl:output method="html" indent="no"/>
    ...
    <xsl:template name="method-single-param"><xsl:if test="position()&gt;1">,&nbsp;</xsl:if><code class="label-method-param"><xsl:value-of select="name"/></code> : <xsl:value-of select="type"/></xsl:template>

    <!-- Method Parameters in short-->
    <xsl:template name="method-params">(<xsl:if test="count(params)&gt;0">&nbsp;</xsl:if><xsl:for-each select="params">
            <xsl:choose>
                <xsl:when test="optional='true'"><span title="Optional" class="optional">[<xsl:call-template name="method-single-param"/>]</span></xsl:when>
                <xsl:otherwise><xsl:call-template name="method-single-param"/></xsl:otherwise>
            </xsl:choose>&nbsp;
        </xsl:for-each>)
    </xsl:template>
    ...
    <xsl:template name="check-if-static">
        <xsl:if test="isStatic='true'"><span class="label-static">&lt;static&gt;</span>&nbsp;</xsl:if>    
    </xsl:template>
